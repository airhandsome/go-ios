...
func Test_device_NewConnect(t *testing.T) {
	...
	if _, err := dev.NewConnect(service.LockdownPort); err != nil {
		t.Fatal(err)
	}
}
...
func Test_device_InstallationProxyBrowse(t *testing.T) {
	...
	list, err := dev.InstallationProxyBrowse(
		service.WithApplicationType(service.ApplicationTypeUser),
		service.WithReturnAttributes("CFBundleDisplayName", "CFBundleIdentifier", "SequenceNumber", "SequenceNumber"),
	)
	// list, err := dev.InstallationProxyBrowse()
	if err != nil {
		t.Fatal(err)
